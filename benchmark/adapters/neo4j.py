...
class Neo4jAdapter(DatabaseAdapter):
    """
    Adapter for self-hosted local Neo4j instance.
    """
    def __init__(self):
        self.uri = os.getenv("NEO4J_URI", "bolt://localhost:7473")
    ...
    def clear_data(self):
        with self.driver.session() as session:
            # Batch edge deletions to prevent transaction memory OOM under 256MB cap
            while True:
            ...
            while True:
                ...
                if val == 0:
                    break
            try:
                session.run("DROP CONSTRAINT user_id_unique").consume()
            except Exception:
                pass
    ...
    def create_indexes(self):
        with self.driver.session() as session:
            try:
                ...
                try:
                    session.run("CREATE CONSTRAINT user_id_unique FOR (u:User) REQUIRE u.id IS UNIQUE").consume()
                except Exception:
                    pass
            try:
                session.run("CREATE INDEX FOR (u:User) ON (u.id)").consume()
            except Exception:
                try:
